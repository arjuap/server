...
exports.handleDirectProductUrl = exports.handleTwilioWebhook = void 0;
const twilio_1 = __importDefault(require("twilio"));
const environment_1 = require("./config/environment");
const product_scraper_1 = require("./services/scraper/product-scraper");
const headless_scraper_1 = require("./services/scraper/headless-scraper");
const ProductService_1 = require("./services/ProductService");
const puppeteer_1 = __importDefault(require("puppeteer"));
// Initialize Twilio client
const twilioClient = (0, twilio_1.default)(environment_1.config.twilio.accountSid, environment_1.config.twilio.authToken);
/**
 ...
 */
const handleDirectProductUrl = async (req, res) => {
    try {
        const { url } = req.body;
        if (!url) {
            res.status(400).json({
...
exports.handleDirectProductUrl = handleDirectProductUrl;
/**
...
async function processProductUrl(url) {
    ...
    try {
        ...
        const isShortUrl = ((_a = siteConfig.shortUrlPatterns) === null || _a === void 0 ? void 0 : _a.some(pattern => url.includes(pattern))) || false;
        ...
        const isProductPage = product_scraper_1.ProductScraperService.isProductPage(processUrl, siteConfig);
        if (!isProductPage) {
            return {
                success: false,
        ...
        const scrapedProduct = await (0, headless_scraper_1.scrapeProductWithPuppeteer)(processUrl);
        ...
        await ProductService_1.productService.storeProduct(scrapedProduct);
        return {
            success: true,
 ...
 */
async function sendTwilioResponse(conversationSid, message) {
    try {
        if (!conversationSid) {
 ...
 */
async function sendTwilioWhatsAppResponse(to, from, message) {
    try {
        if (!to || !from) {
